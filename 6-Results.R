...
### CEA table - Long
  ...
  colnames(cea_table_long) <- c("Category", "Arm",
                                ...
                                " ",
                                "ICER", 
                                "INMB", "INMB_alt", "INMB_alt_mcse",
                                "INHB", "INHB_alt", "INHB_alt_mcse",
                                " ",
                                "n") 
  ...
  for (i in seq_along(unique_combinations$variable)) {
    ...
    for (category in ordered_categories) {
      ...
      cea_table_row <- data.frame(
        ...
        , Category = category
        , Incremental_Costs = delta.C #  round(delta.C, 0)
        , Incremental_QALYs = delta.E #  round(delta.E, 3)
        , ICER = round(ICER, 0)
        , INHB = INHB
...
ceac_plot <- create_ceac_plot(ceac, "inc_cost", "inc_effect", "Base Case")

pr_CE_base <- (ceac_plot$data$pr_CE[ceac_plot$data$wtp_range == wtp])*100

cat("Medicaid expansion has a", pr_CE_base, "% probability of being cost-effective at $", wtp, "/WTP", "\n")
...
#### Distributional Analysis ----
{
  ...
  subset_cost_data <- summarised_output %>%
    filter(category == "Total") %>%
  ...
  #Identify the proportion of replications that are cost-effective and equitable for pop
  ede_data_pr_pop <- combined_output %>%
    ...
    mutate(  nhb_ntrt = (te_ntrt) - ((tc_ntrt) / wtp)
             ...
             , inhb     = nhb_trt - nhb_ntrt) %>%
    group_by(dataset_index, replication, variable) %>%
    summarise(  ede_ntrt    = (calculate_ede(nhb_ntrt, epsilon, kp = FALSE, population_size = num_individuals)) 
                , ede_trt     = (calculate_ede(nhb_trt, epsilon, kp = FALSE, population_size = num_individuals))
                , nhb_ntrt    = weighted.mean(nhb_ntrt, num_individuals, na.rm = TRUE) 
                , nhb_trt     = weighted.mean(nhb_trt, num_individuals, na.rm = TRUE)  
    ...
    ungroup() %>%
    group_by(variable) %>%
    mutate(pr_ce = ifelse(inhb > 0, 1, 0),
           pr_eq = ifelse(Ae > 0, 1, 0),
           pr_ce_eq = ifelse((Ae > 0) & (inhb > 0), 1, 0)) %>%
    summarise(pr_ce = sum(pr_ce)/length(pr_ce)
              , pr_eq = sum(pr_eq)/length(pr_eq)
              , pr_ce_eq = sum(pr_ce_eq)/length(pr_ce_eq))
  ...
  ede_plot_pop <- ggplot(ede_data_pop, aes(x = pop_ineq_burden, y = pop_inhb, color = variable, shape = variable, label = custom_labels)) +
    ...
    coord_cartesian(
      xlim = c(
        0 - min(ede_data_pop$pop_ineq_burden) * 0.3, 
        max(ede_data_pop$pop_ineq_burden) + max(ede_data_pop$pop_ineq_burden) * 0.1
      ),
      ylim = c(
        0 - min(ede_data_pop$pop_inhb) * 0.01, 
        max(ede_data_pop$pop_inhb) + max(ede_data_pop$pop_inhb) * 0.1
      )
    )
  ...
  for (v in variable_values) {
    ...
    for (i in seq_along(wtp_values)) {
      ...
      for (j in seq_along(epsilon_values)) {
        ...
        if (any(!is.numeric(edeh_data$edeh) | is.na(edeh_data$edeh) | !is.finite(edeh_data$edeh))) {
          cat("Problematic combination: wtp =", wtp_val, "epsilon =", epsilon, "\n")
        ...
        edeh_data$CE_threshold <- wtp_val
    ...
    subset_data <- dcea_data[dcea_data$CE_threshold == wtp_val, ]
    ...
    cat("For", name
        , "Medicaid expansion would be cost-effective (at a value of $", wtp, "/WTP) if epsilon was"
...
ggplot(
  ...
  aes(x = replication, y = inhb, group = 1)) +  # Specify grouping
  geom_line() + 
...
if (.one_way == 1) {
  ...
  variable_names <- c(
    ...
    "cost_routine$mean",
    "delta_routine_prop$mean",
    "cost_drugs_cvd_diab",
    "delta_drugs_cvd$mean",
    "delta_drugs_diab$mean",
    "annual_earnings",
    "u$mi$mean",
    "u$stroke$mean",
    "d.e",
  ...
  plot_names <- c(
    ...
    "Routine visit cost",
    "Delta Routine visit cost",
    "CVD and Diabetes drugs cost",
    "Delta Diabetes drugs cost",
    "Delta CVD drugs cost",
    "Annual earnings",
    "Utility-MI",
    "Utility-Stroke",
  ...
  # Calculate the average INHB as a single numeric value
  average_INHB <- tornado_data %>%
    mutate(mean = ((Lower_INHB) + (Upper_INHB)) / 2) %>%
    summarise(average_INHB = mean(mean)) %>%
    pull(average_INHB)
...
if (file.exists(here("table", results_tables))) {
  file.remove(here("table", results_tables))
}

# Save the Excel workbook
saveWorkbook(wb, file = (here("tables", results_tables)))
...
ggplot_list <- list(
  combined_INHB_plots
  , combined_cost_plots
  , combined_effect_plots
  , stacked_cost_plot
  , nhb_charts
  , cost_charts
  , qaly_charts
  , ede_plot_pop
  , ce_planes_list$Total
  , ce_planes_list$age_cat
  , ce_planes_list$race
  , ce_planes_list$fam_income
  , ce_planes_list$education
...
output_pptx <- file.path(here("figures", results_graphs))
...
saveRDS(combined_output, file = (here("data", "output_data", results_combined_output)))
