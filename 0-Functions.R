...
cipreme_packages <- function(){
  ...
  
  {
    pacman::p_load(
      ...
      "fastDummies", #to create dummy variables: dummy_cols("var", remove_selected_columns = F)
...

{
  ...
  Costs <- function(mydata,
    ...
    mydata  <- mydata %>%
      mutate( # calculate the odds of having non-zero health expenditures 
        ...
        delta_total_hc_cost = case_when((Trt == TRUE & .medicaid_cost == 1) ~ 
                                          (((parameters[["c_routine"]] * parameters[["delta_c_routine"]]) +
                                              (cost_drugs_cvd_diab * (parameters[["delta_d_diab"]] + parameters[["delta_d_cvd"]])))
                                           * medicaid_exp)
                                        , (Trt == FALSE) ~ 0
                                        , TRUE ~ 0),
  ...
  generate_parameters <- function(Deterministic, ci2se = 3.92) {
    ...
    # Annual per person cost of routine visits [See LB's inputs excel file for calculations]
    parameter_list[["c_routine"]]             <- ifelse(Deterministic == 1, cost_routine$mean, rgamma(1, shape = cost_routine$shape, rate = cost_routine$rate)) * pce_all_2016
    
    # Proportionate increase in cost of routine visits following Medicaid expansion [See LB's inputs excel file for calculations]
    parameter_list[["delta_c_routine"]]       <- ifelse(Deterministic == 1, delta_routine_prop$mean, rbeta(1, delta_routine_prop$shape1, delta_routine_prop$shape2))
    
    # change in cvd prescription spending following Medicaid expansion
    parameter_list[["delta_d_cvd"]]          <- ifelse(Deterministic == 1, delta_drugs_cvd$mean, rbeta(1, delta_drugs_cvd$shape1, delta_drugs_cvd$shape2))
    
    # change in cvd prescription spending following Medicaid expansion
    parameter_list[["delta_d_diab"]]          <- ifelse(Deterministic == 1, delta_drugs_diab$mean, rbeta(1, delta_drugs_diab$shape1, delta_drugs_diab$shape2))
  ...
  Bootstrap_msm <- function(original_data 
    ...
    bootdata_original <- bootdata_original %>%
      # filter to just those receiving the treatment if .ATT == 1, otherwise the whole sample 
      filter(medicaid_exp == 1 | .ATT != 1)

    # Randomly select replicate weights from columns V1 to V100
    ...
    bootdata_original <- slice_sample(bootdata_original, prop = 1, weight_by = !!sym(random_col), replace = T) %>%
      # remove replicate weights
      select(-starts_with("V")) # drop replicate weights
  ...
  dsa <- function(perturbed_values) {
    ...
    odds_hc$mi$mean         <<- perturbed_values[1]
    odds_hc$stroke$mean     <<- perturbed_values[2]
    hc_cost$mi$mean         <<- perturbed_values[3]
    hc_cost$stroke$mean     <<- perturbed_values[4]
    cost_absenteeism$mean   <<- perturbed_values[5]
    cost_disability$mean    <<- perturbed_values[6]
    delta_c_y1$mean         <<- perturbed_values[7]
    cost_routine$mean       <<- perturbed_values[8]
    delta_routine_prop$mean <<- perturbed_values[9]
    cost_drugs_cvd_diab     <<- perturbed_values[10]
    delta_drugs_cvd$mean    <<- perturbed_values[11]
    delta_drugs_diab$mean   <<- perturbed_values[12]
    c_admin_enrollee$mean   <<- perturbed_values[13]
    annual_earnings         <<- perturbed_values[14]
    u$mi$mean               <<- perturbed_values[15]
    u$stroke$mean           <<- perturbed_values[16]
    d.e                     <<- perturbed_values[17]
    d.c                     <<- perturbed_values[18]
    #delta_CVD_history$mean  <<- perturbed_values[19]
    #delta_hba1c$mean        <<- perturbed_values[20]
    #delta_sys_bp$mean       <<- perturbed_values[21]
    ...
    for (dataset_index in 1:m) {
      ...
      for (replication in 1:bootsize) {
        ...
        dsa_result$replication   <- replication
        dsa_result$dataset_index <- dataset_index
        
        # Append the results to the combined data frame
        dsa_combined_output <- bind_rows(dsa_combined_output, dsa_result)
    ...
    for (imputation_index in current_m:m) {
  ...
  create_ceac_plot <- function(data, cost_var, effect_var, title_suffix) {
    ...
    calculate_pr_ce <- function(wtp_temp) {
      pr_CE <- ifelse(((data[[effect_var]] * wtp_temp) - data[[cost_var]]) > 0, 1, 0)
      mean(pr_CE)
    }
    ...
    ggplot(ceac_plot_data, aes(x = wtp_range)) +
      ...
      geom_vline(xintercept = wtp, linetype = "dashed", color = "black") +
      annotate("text", x = wtp, y = 0.05, label = "$150,000", vjust = 0.5, hjust = -0.05, color = "black") +
